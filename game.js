...
const SAVE_KEY = 'lemonadeEmpireSave';
const TICK_MS = 100;
...
function tick() {
  ...
  // --- sales ---
  const demandRaw = 100 * state.marketingLevel * state.marketingEffMult /
    (1 + (state.price * 20) / state.priceToleranceMult) * state.demandMult;
  state.demand = Math.max(1, Math.min(100, demandRaw));
  const maxSaleRate = state.marketingLevel * 2 * (state.demand / 100) * 50 * state.salesRateMult;
...
function fastRender() {
  $('cupsSold').textContent = fmt(state.cupsSold);
  $('cupsPerSecond').textContent = fmt(state.lastProduced || 0);
  $('cupsUnsold').textContent = fmt(state.cupsUnsold);
...
function wireEvents() {
  ...
  $('btnSqueeze').addEventListener('click', () => {
    if (state.lemons >= 1) {
      state.lemons -= 1;
      const made = state.lemonYieldMult;
      state.cupsMade += made;
